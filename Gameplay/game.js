...
let allowNegativeTimeMode = false;
const __activeIntervals = [];
function safeSetInterval(fn, ms) {
    const id = safeSetInterval(fn, ms);
    __activeIntervals.push(id);
    return id;
}
...
// fallback tick used by some buffs
function startTimerTick() {
    if (typeof timeLeft === 'number') {
        timeLeft = Math.max(0, timeLeft - 1);
        const tEl = document.getElementById("timer"); if (tEl) tEl.textContent = Math.max(0, Math.round(timeLeft));
    }
}
...
async function fetchParagraph() {
    // reset base mechanics to avoid buff carryover between rounds
    gameMechanics.timerBase = GLOBAL_TIMER;
    gameMechanics.healthMax = 100;
    gameMechanics.health = 100;
    gameMechanics.healthLossPerWrong = 1;
    gameMechanics.scorePerCorrectChar = 10;
...
function showDeathScreen(reason = "unknown") {
    ...
    clearActiveIntervals(); revertLegendaryTheme(); let deathScreen = document.getElementById("death-screen");
    if (!deathScreen) {
...
function applyBuffEffects(buff) {
    ...
    switch (buff.name) {
        ...
        case "Made in Heaven": {
            ...
            const timerEl = document.getElementById("timer");
            clearInterval(timerInterval); timerInterval = null; allowNegativeTimeMode = true;
        ...
        case "Pierre-Auguste Renoir":
            ...
            if (!document.getElementById('renoir-pulse')) {
                const overlay = document.createElement("div"); overlay.id = "renoir-pulse";
        ...
        case "Lelouch":
            ...
            const lelouchInterval = safeSetInterval(() => {
        ...
        case "Dream On":
            ...
            const dreamCheck = safeSetInterval(() => {
                if (!dreamFrozen && timeLeft <= 30 && timeLeft > 25) {
                    ...
                    setTimeout(() => {
                        ...
                        timerInterval = safeSetInterval(startTimerTick, 1000);
                    }, 5000);
                }
            }, 1000);
            break;
        ...
        case "Thick of It":
            const thickInterval = safeSetInterval(() => {
        ...
        case "Photosynthesis":
            const photoInterval = safeSetInterval(() => {
        ...
        case "Ultra Omega Extreme Demon":
            const checkDemon = safeSetInterval(() => {
        ...
        case "Hello Everybody":
            ...
            const helloInterval = safeSetInterval(() => {
...
function applyLegendaryTheme(buffName) {
    ...
    function showRenoirSubtitles(audioElement) {
        ...
        renoirSubtitleInterval = safeSetInterval(() => {
    ...
    if (buffName === "Pierre-Auguste Renoir") {
        ...
        if (!document.getElementById('renoir-vignette')) {
            const vignette = document.createElement("div"); vignette.id = "renoir-vignette";
        ...
        if (!document.getElementById('renoir-rays')) {
            const rays = document.createElement("div"); rays.id = "renoir-rays";
        ...
        if (!document.getElementById('renoir-bloom')) {
            const bloom = document.createElement("div"); bloom.id = "renoir-bloom";
