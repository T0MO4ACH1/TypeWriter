...
const buffs = [
    ...
    { name: "Pierre-Auguste Renoir", rarity: "legendary", price: 300, description: "health locked at 30% max but heals 3 HP and adds +3 seconds to the timer", reference: " Hate me all you want, as long as you're alive to do it.", icon: "fa-solid fa-palette" },
...
const currentBuffBox = document.getElementById('currentBuffBox');
const currentBuffIcon = document.getElementById('currentBuffIcon');
const currentBuffName = document.getElementById('currentBuffName');
const currentBuffDesc = document.getElementById('currentBuffDesc');
const sellBuffBtn = document.getElementById('sellBuffBtn');
...
const rarityConfig = {
    ...
    legendary: { weight: 0.020, class: 'rar-legendary', colorVar: '--purple' },
    rare: { weight: 0.040, class: 'rar-rare', colorVar: '--red' },
    uncommon: { weight: 0.44, class: 'rar-uncommon', colorVar: '--green' },
    common: { weight: 0.50, class: 'rar-common', colorVar: '--blue' }
};
...
function renderCurrentBuff() {
    const equipped = localStorage.getItem('equippedBuff');
    if (!equipped) {
        currentBuffBox.classList.add('hidden');
        return;
    }

    const buff = JSON.parse(equipped);
    currentBuffBox.classList.remove('hidden');
    currentBuffIcon.innerHTML = `<i class="${buff.icon}"></i>`;
    currentBuffName.textContent = buff.name;
    currentBuffDesc.textContent = buff.description;

    // sell logic
    sellBuffBtn.onclick = () => {
        const ownedKey = 'owned::' + buff.name;
        localStorage.removeItem('equippedBuff');
        localStorage.removeItem(ownedKey);

        coins += Math.floor(buff.price * 1);
        localStorage.setItem('playerCoins', coins.toString());
        coinsEl.textContent = coins;

        // hide box & refresh shop
        currentBuffBox.classList.add('hidden');
        const newOffers = generateOffers(3);
        renderOffers(newOffers);
        renderCurrentBuff();
